...
def is_supported_in_ld3(x, y, z, dx, dy, dz, grid, threshold=0.7, give_ratio = False):
    if z == 0:
        return True  # Base layer is always supported 
    elif 0.819 * z + 61.475 - 0.5 < (x+dx) < 0.819 * z + 61.475 + 0.5:
        return True
...
def ld3_checker(x, y, z, dx, dy, dz, grid, threshold = 0.9):
    return (
        is_box_inside_ld1(x, y, z, dx, dy, dz)
        and is_supported_in_ld3(x, y, z, dx, dy, dz, grid, threshold)
    )
                    ...
                    for y in range(ly - dy + 1):
                        for i in range(0, len(rotations)):
                            ...
                            if (np.all(grid[z:z+dz, y:y+dy, x:x+dx] == 0) and
                                is_box_inside_ld1(x, y, z, dx, dy, dz) and
                                is_supported_in_ld3(x, y, z, dx, dy, dz, grid, threshold=0.9)
                                    ):
